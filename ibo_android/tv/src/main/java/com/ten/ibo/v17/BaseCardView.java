...
public class BaseCardView extends FrameLayout {
    ...
    private void findChildrenViews() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            BaseCardView.LayoutParams lp = (BaseCardView.LayoutParams) child
                    .getLayoutParams();
            if (lp.viewType == BaseCardView.LayoutParams.VIEW_TYPE_INFO) {
                child.setAlpha(mInfoAlpha);
                mInfoViewList.add(child);
                child.setVisibility(infoVisible ? View.VISIBLE : View.GONE);
            } else if (lp.viewType == BaseCardView.LayoutParams.VIEW_TYPE_EXTRA) {
    ...
    void animateInfoOffset(boolean shown) {
        ...
        mAnim = new BaseCardView.InfoOffsetAnimation(mInfoOffset, shown ? extraHeight : 0);
    ...
    private void animateInfoHeight(boolean shown) {
        ...
        mAnim = new BaseCardView.InfoHeightAnimation(mInfoVisFraction, targetFraction);
        mAnim.setDuration(mSelectedAnimDuration);
        mAnim.setInterpolator(new AccelerateDecelerateInterpolator());
        mAnim.setAnimationListener(new Animation.AnimationListener() {
            @Override
    ...
    private void animateInfoAlpha(boolean shown) {
        ...
        mAnim = new BaseCardView.InfoAlphaAnimation(mInfoAlpha, shown ? 1.0f : 0.0f);
    ...
    @Override
    public BaseCardView.LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new BaseCardView.LayoutParams(getContext(), attrs);
    }

    @Override
    protected BaseCardView.LayoutParams generateDefaultLayoutParams() {
        return new BaseCardView.LayoutParams(
                BaseCardView.LayoutParams.WRAP_CONTENT, BaseCardView.LayoutParams.WRAP_CONTENT);
    }

    @Override
    protected BaseCardView.LayoutParams generateLayoutParams(ViewGroup.LayoutParams lp) {
        if (lp instanceof BaseCardView.LayoutParams) {
            return new BaseCardView.LayoutParams((BaseCardView.LayoutParams) lp);
        } else {
            return new BaseCardView.LayoutParams(lp);
        }
    }
    ...
    final class InfoOffsetAnimation extends BaseCardView.AnimationBase {
    ...
    // for the info fields.
    final class InfoHeightAnimation extends BaseCardView.AnimationBase {
        private float mStartValue;
        private float mDelta;
    ...
    final class InfoAlphaAnimation extends BaseCardView.AnimationBase {
        private float mStartValue;
        private float mDelta;
